...
import { Column, SelectField } from "@hakit/addon/components";
...
function Render(props: RenderProps<DateProps>) {
  ...
  const sep = token.sep === ' ' ? '\u00A0' : (token.sep === ', ' ? ',\u00A0' : token.sep);
  return <div key={i} className="ha-date-text__sep" suppressHydrationWarning>{sep}</div>;
      })}
    </div>
  );
}
